...
def render_learning_session():
    ...
    # Testing shortcuts (only for admin or debug mode)
...
def render_quiz():
    ...
    if not st.session_state.quiz_submitted:
        ...
        if not all_answered:
            st.info(
                f"Please answer all questions ({len(st.session_state.quiz_answers)}/{len(quiz_questions)} complete)")
...
def main():
    ...
    if st.query_params.get("debug") == "true":
        from firebase_debug import render_debug_dashboard
